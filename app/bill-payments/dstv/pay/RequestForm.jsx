...
export default function Init(){
    ...
    const handleValidate = async(e)=>{
        ...
        const res = await fetch(API_URL + 'zba/dstv/', {
            method: 'POST',
            body: JSON.stringify({
                billerId: "DSTV",
            ...
            if ( data.success ) {
                ...
                // TODO: alert error
                showErrorModal(`success`, 'The system is not available right now, please try again later.');
            }
            setIsLoading(false);
        }).catch((error) => {
            // TODO: alert error
    ...
    const handlePayment = async(e)=>{
        ...
        window.location.href = POMPAY_URL+'?q=dstv&p='+phone+'&e='+email+'&c='+selectedCurrency+'&a='+amount+'&cn='+details.CustomerName+'&ca='+account;
    }
                    ...
                    <div className="card lg:card-side bg-base-100 shadow-xl">
                      ...
                      <div className="card-body">
                          <form onSubmit={handleValidate}>
                              <h2 className="p24-section-heading">Enter Account Number</h2>
                                ...
                                {details && (
                                    <div className="space-y-2">
                                        <div className="flex flex-col">
                                            <label className="text-sm font-semibold text-accent">account holder</label>
                                            <span className="text-primary font-semibold">{details.CustomerName}</span>
                                        </div>
                                        <div className="flex flex-col">
                                            <label className="text-sm font-semibold text-accent">amount due</label>
                                            <span className="text-primary font-semibold">{details.AmountDue}</span>
                                        </div>
                                        <div className="flex flex-col">
                                            <label className="text-sm font-semibold text-accent">currency</label>
                                            <span className="text-primary font-semibold">{details.Currency}</span>
                                        </div>
                                        <div className="flex flex-col">
                                            <label className="text-sm font-semibold text-accent">due date</label>
                                            <span className="text-primary font-semibold">{details.DueDate}</span>
                                        </div>
                                    </div>
                                )}
                            </div>
